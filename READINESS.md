...
| API errors return structured JSON | ⚠️ | 2026-05-20 | Cloudflare launch backend now has a top-level JSON error boundary. Live probes returned structured JSON for unknown brand, unknown path, unauthorized session, invalid signup, invalid Stripe signature, and rate-limited auth. Full upstream API error surfaces are not hosted/proven. |
| Background jobs and cron tasks verified | ⚠️ | 2026-05-20 | Cloudflare monitor cron is configured every 5 minutes. Manual /monitor/run persisted 9 checks to D1 and /monitor/status returned current rows; it correctly flags mathematricks-owned as failing. Full upstream app background jobs are not hosted/proven. |
...
| Application logs queryable; PII not logged | ⚠️ | 2026-05-20 | Cloudflare launch backend now writes D1 request logs and exposes /logs/status. Live probe returned grouped request logs plus a PII policy confirming only method, path without query string, status, duration, ok flag, brand, and timestamp are stored; bodies, query strings, emails, auth tokens, IPs, and card data are not logged. Full upstream app logs remain unproven until monolith hosting exists. |
